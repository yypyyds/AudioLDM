...
class AutoencoderKL(nn.Module):
    ...
    def encode(self, x):
        ...
        x = self.freq_split_subband(x)  # 频域分离子带，但是第一轮全是0
        h = self.encoder(x)             # VAE的encoder，但是第一轮传入的是全0
        moments = self.quant_conv(h)    # 一个二维卷积改了维度，kernel为1，就是一个线性投影层
        posterior = DiagonalGaussianDistribution(moments)   # 生成均值和方差
        return posterior

    def decode(self, z):                # VAE解码，得到mel谱图
        z = self.post_quant_conv(z)     # 线性投影层
        dec = self.decoder(z)           # VAE的decoder
        dec = self.freq_merge_subband(dec)  # 梅尔滤波器组的操作，但是这里没传参数
        return dec
