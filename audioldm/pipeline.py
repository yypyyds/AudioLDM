...
def text_to_audio(
    ...
    seed_everything(int(seed))  # 将所有用到随机数的函数用seed重置随机数根
    waveform = None
    if(original_audio_file_path is not None):
        waveform = read_wav_file(original_audio_file_path, int(duration * 102.4) * 160)
        
    batch = make_batch_for_text_to_audio(text, waveform=waveform, batchsize=batchsize)  # 生成文本批次，应该是批量生成音频用的

    latent_diffusion.latent_t_size = duration_to_latent_t_size(duration)    # 来自ldm.LatentDiffusion，继承自DDPM
    ...
    if(waveform is not None):
        ...
        latent_diffusion = set_cond_text(latent_diffusion)  # 设置条件的文本
        
    with torch.no_grad():
        waveform = latent_diffusion.generate_sample(    # LDM
            [batch],
            unconditional_guidance_scale=guidance_scale,        # 指导参数，用于随机丢弃E^y
            ddim_steps=ddim_steps,                              # 暂时还不知道是干嘛的
            n_candidate_gen_per_text=n_candidate_gen_per_text,  # 每个文本生成多少条音频，然后挑选质量最好的
            duration=duration,                                  # 生成音频的长度
        )
    return waveform
