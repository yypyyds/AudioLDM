...
class LatentDiffusion(DDPM):
    ...
    def get_input(
        ...
        x = super().get_input(batch, k) # 随机噪声的输入
        ...
        if return_first_stage_encode:
            ...
            z = self.get_first_stage_encoding(encoder_posterior).detach()   # 应该或许是 z_n
        else:
            z = None
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key != self.first_stage_key:
                if cond_key in ["caption", "coordinates_bbox"]:
                    ...
                    # [bs, 1, 527]
                    xc = super().get_input(batch, cond_key)     # 条件噪声的输入
            ...
            if not self.cond_stage_trainable or force_c_encode:
                if isinstance(xc, dict) or isinstance(xc, list):
                    ...
                    c = self.get_learned_conditioning(xc.to(self.device)) # 这一步将条件text变成向量
            else:
                c = xc
    ...
    def generate_sample(
        ...
        **kwargs,
        ...
        """
        batchs: text的一些信息
        ddim_steps: 扩散模型的参数 N
        n_candidate_gen_per_text: 每个文本生成多少条音频，然后挑选质量最好的
        unconditional_guidance_scale: 扩散模型指导参数，调控E^y条件噪声与随机噪声的比例
        """
        ...
        with self.ema_scope("Generate"):    # 指数移动平均
            for batch in batchs:
                z, c = self.get_input(
                    batch,
                ...
                text = super().get_input(batch, "text") # DDPM父类的get_input，这个get_input就是判断
                                                        # 现在进行的是什么任务，返回对应的条件
                ...
                samples, _ = self.sample_log(           # LDM过程，得到VAE的隐变量
                    cond=c,
                    batch_size=batch_size,
                    x_T=x_T,
                    ddim=use_ddim,
                ...
                mel = self.decode_first_stage(samples)      # VAE的decoder，得到梅尔谱图

                waveform = self.mel_spectrogram_to_waveform(mel)    # HiFi-GAN，得到波形
